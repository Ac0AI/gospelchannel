...
export function SpotifyPlaylistShelf({
  ...
  return (
    <section className="rounded-3xl border border-rose-200/60 bg-gradient-to-br from-white to-blush-light/50 p-5 shadow-sm sm:p-8">
      ...
      <div className="scrollbar-hide -mx-5 flex gap-4 overflow-x-auto px-5 sm:-mx-8 sm:px-8 lg:mx-0 lg:grid lg:grid-cols-1 lg:overflow-visible lg:px-0 xl:grid-cols-2" style={{ scrollSnapType: "x mandatory" }}>
        {featured.map((item) => (
          <article
            key={item.playlistId}
            className="flex w-[280px] shrink-0 flex-col overflow-hidden rounded-2xl border border-rose-200/70 bg-white/85 shadow-sm lg:w-auto"
            ...
            <div className="flex items-start justify-between gap-3 border-b border-rose-100 px-4 py-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {item.tag ? (
                    <span className="inline-flex rounded-full bg-blush-light px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-rose-gold-deep">
                      {item.tag}
                    </span>
                  ) : null}
                </div>
                <h3 className="mt-1 truncate font-serif text-base font-semibold text-espresso">{item.title}</h3>
              ...
              <a
                ...
                className="shrink-0 self-center rounded-full border border-blush px-3 py-1.5 text-xs font-semibold text-warm-brown transition-colors hover:bg-blush-light hover:text-espresso"
              >
                Open ↗
              </a>
            </div>
